    ...
    <div class="card-body ">
        <div class="row">
            <div class="col-sm-6 float-left">
                <legend class="float-left">Transportes</legend>
            </div>
        ...
        <div class="card-block mt-4">
            <div id="Transportes" class="table-responsive">
                <table id="tblTransportes" class="table table-sm display " style="width:100%">
                    <thead>
                        <tr>
                            <th>ID</th>
                            <th>Clave</th>
...
<script>
    var master_url = base_url + 'index.php/Transportes/';
    var tblTransportes = $('#tblTransportes');
    var Transportes;
    ...
    function getRecords() {
        ...
        if ($.fn.DataTable.isDataTable('#tblTransportes')) {
            tblTransportes.DataTable().destroy();
        }
        Transportes = tblTransportes.DataTable({
            "dom": 'Bfrtip',
            buttons: buttons,
            "ajax": {
        ...
        $('#tblTransportes_filter input[type=search]').focus();

        tblTransportes.find('tbody').on('click', 'tr', function () {
            HoldOn.open({
                theme: 'sk-cube',
                message: 'CARGANDO...'
            });
            nuevo = false;
            tblTransportes.find("tbody tr").removeClass("success");
            $(this).addClass("success");
            var dtm = Transportes.row(this).data();
            temp = parseInt(dtm.ID);
            $.getJSON(master_url + 'getTransporteByID', {ID: temp}).done(function (data) {
                pnlDatos.find("input").val("");
                $.each(pnlDatos.find("select"), function (k, v) {
                    pnlDatos.find("select")[k].selectize.clear(true);
                });
