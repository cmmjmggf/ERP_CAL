    ...
    <div class="card-body ">
        <div class="row">
            <div class="col-sm-6 float-left">
                <legend class="float-left">Sub Fracciones</legend>
            </div>
        ...
        <div class="card-block mt-4">
            <div id="Subfracciones" class="table-responsive">
                <table id="tblSubfracciones" class="table table-sm display " style="width:100%">
                    <thead>
                        <tr>
                            <th>ID</th>
                            <th>Clave</th>
...
<script>
    ...
    var tblSubfracciones = $('#tblSubfracciones');
    var Subfracciones;
    ...
    function getRecords() {
        ...
        if ($.fn.DataTable.isDataTable('#tblSubfracciones')) {
            tblSubfracciones.DataTable().destroy();
        }
        Subfracciones = tblSubfracciones.DataTable({
            "dom": 'Bfrtip',
            buttons: buttons,
            "ajax": {
        ...
        $('#tblSubfracciones_filter input[type=search]').focus();

        tblSubfracciones.find('tbody').on('click', 'tr', function () {
            HoldOn.open({
                theme: 'sk-cube',
                message: 'CARGANDO...'
            });
            nuevo = false;
            tblSubfracciones.find("tbody tr").removeClass("success");
            $(this).addClass("success");
            var dtm = Subfracciones.row(this).data();
            temp = parseInt(dtm.ID);
            $.getJSON(master_url + 'getSubFraccionByID', {ID: temp}).done(function (data) {
                pnlDatos.find("input").val("");
                $.each(pnlDatos.find("select"), function (k, v) {
                    pnlDatos.find("select")[k].selectize.clear(true);
                });
