<div class="modal " id="mdlEstiloFraccionNomina"  role="dialog">
...
<script>
        ...
        mdlEstiloFraccionNomina.find("#Ano").change(function () {
            if (parseInt($(this).val()) < 2015 || parseInt($(this).val()) > 2025 || $(this).val() === '') {
                swal({
                    ...
                    mdlEstiloFraccionNomina.find("#Ano").val("");
                    mdlEstiloFraccionNomina.find("#Ano").focus();
                });
            }
        });
        mdlEstiloFraccionNomina.find("#Maq").change(function () {
            onComprobarMaquilas($(this));
        });
        mdlEstiloFraccionNomina.find("#Sem").change(function () {
            var ano = mdlEstiloFraccionNomina.find("#Ano");
    ...
    function onImprimirReporteConciliaFraccion() {
        ...
        $.ajax({
            url: base_url + 'index.php/ReportesProduccionJasper/onReporteConciliaSemanaFraccionEstilo',
            type: "POST",
            cache: false,
            contentType: false,
            processData: false,
