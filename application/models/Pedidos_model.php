...
class Pedidos_model extends CI_Model {
    ...
    public function getRecords() {
        try {
            return $this->db->select("P.ID, P.Clave, P.Cliente AS Cliente, P.Agente Agente,P.FechaPedido,SUM(P.Pares) AS Pares", false)
                            ->from('pedidox AS P')->group_by('P.Clave')->order_by('P.FechaPedido', 'DESC')->get()->result();
        } catch (Exception $exc) {
    ...
    public function getPedidoByPedidoByCliente($cliente, $pedido) {
        try {
            return $this->db->select('P.ID AS ID, P.Clave')
                            ->from('pedidox AS P')
                            ->where('P.Clave', $Clave)
                            ->where('P.Estatus', 'A')->get()->result();
        } catch (Exception $exc) {
            echo $exc->getTraceAsString();
        }
    }
...
//            print $this->db->last_query();
    ...
    public function getPedidoDByID($ID) {
        try {
