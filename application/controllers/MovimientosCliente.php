...
class MovimientosCliente extends CI_Controller {
    ...
    public function onReporteAntiguedadSaldosPorCliente() {
        ...
        $Clientes = $this->getClientesReporteAntiguedad($Cte, $aCte);
        $Doctos = $this->getDoctosByClientesTpAntiguedad($Cte, $aCte);
        ...
        if (!empty($Clientes)) {
            ...
            foreach ($Clientes as $key => $G) {
                ...
                $pdf->Cell(90, 5, utf8_decode($G->ClienteF . ' =====> PLAZO: ' . $G->Plazo . ' DÍAS'), 'B'/* BORDE */, 1, 'L');
    ...
    public function getClientesReporteAntiguedad($cte, $acte) {
        try {
    ...
    public function getDoctosByClientesTpAntiguedad($cte, $acte) {
        try {
