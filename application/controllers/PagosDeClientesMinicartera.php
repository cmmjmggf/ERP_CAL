...
class PagosDeClientesMinicartera extends CI_Controller {
    ...
    public function onRedondeaYActualizaSaldos() {
        try {
            $this->db->query(" update cartclientem set saldo = 0, status = 3 where saldo < 1 and saldo > 0; ");
            $this->db->query(" update cartclientem set saldo = 0, status = 3 where saldo < 0 and saldo > -1; ");
        } catch (Exception $exc) {
