...
//                                    ->get()->result());
            ...
            $DEPTOS_FISICOS = array(15,20, 30, 40/* PREL-CORTE */, 60, 120, 140, 70, 80/* RAYADO CONTADO */, 90/* ENTRETELADO */, 140/* ENSUELADO */, 300 /* SUPERVISORES */);
            $xXx = $this->input->post();
            ...
            $this->db->select("CONCAT(E.PrimerNombre,' ',"
                            ...
                            . "' ',(CASE WHEN E.Paterno <>'0' THEN E.Paterno ELSE '' END),' ',"
                            . "(CASE WHEN E.Materno <>'0' THEN E.Materno ELSE '' END)) AS NOMBRE_COMPLETO, "
                            . "E.DepartamentoFisico AS DEPTOCTO, D.Avance AS GENERA_AVANCE, D.Descripcion AS DEPTO, E.Foto AS FOTO_EMPLEADO", false)
...
//            exit(0);
            foreach ($FRACCIONES as $k => $v) {
                $FRACCION = intval($v->NUMERO_FRACCION);
                ...
                if ($XMAQUILA === 1 && $FRACCION === 61) {
                    $FRACCION = 60;
                }
                /* CONDICIONES INTERNAS - ENTRETELADO */
                if ($XMAQUILA === 1 && $FRACCION === 127) {
                    $FRACCION = 51;
                }
                /* CONDICIONES INTERNAS - ENSUELADO */
                if ($XMAQUILA === 1 && $FRACCION === 396) {
                    $FRACCION = 397;
                }
                /* CONDICIONES INTERNAS
                 * 
                 * 606	ARM.PLANTILLA ADORNO MUESTRA 
                 * 607	ARMAR PLANTILLA DE ADORNO
                 * 
                 *  */
                if ($XMAQUILA === 1 && $FRACCION === 606) {
                    $FRACCION = 607;
                }
                if ($XMAQUILA === 1 && $FRACCION === 23) {
                    $FRACCION = 24;
                }
                /* CONDICIONES INTERNAS */
...
//                var_dump($check_fraccion,$check_fraccion_fxe);
                ...
                if ($check_fraccion[0]->EXISTE <= 0 && intval($check_fraccion_plantilla[0]->EXISTE) <= 0 && $check_fraccion_fxe[0]->EXISTE > 0) {
                    $PRECIO_FRACCION_CONTROL = $this->db->query("SELECT A.Estilo, A.Pares, FXE.CostoMO, (A.Pares * FXE.CostoMO) AS TOTAL FROM pedidox AS A INNER JOIN fraccionesxestilo as FXE ON A.Estilo = FXE.Estilo WHERE  FXE.Fraccion = {$FRACCION} AND A.Control = {$xXx['CONTROL']}")->result();
                    ...
                    if ($FRACCION !== 60 && $FRACCION !== 61 && $FRACCION !== 71) {
                        switch ($FRACCION) {
                            case 127:
                            case 51:
                                $EMPLEADO = intval($xXx['NUMERO_EMPLEADO']);
                                $EMPLEADOS_PERMITIDOS = ARRAY(2160, 3172);
                                if (in_array($EMPLEADO, $EMPLEADOS_PERMITIDOS)) {
...
//                                            $check_avance = $this->db->query("SELECT * FROM avance WHERE Control = {} AND Departamento = 105")->result();
                                            ...
                                            $fraccion_existe = $this->db->query("SELECT COUNT(*) AS EXISTE FROM fracpagnomina AS F WHERE F.numfrac IN(51,127) AND F.control = {$xXx['CONTROL']}")->result();
                                ...
                                break;
                            case 506:
                                $l = new Logs("AVANCE 8", "FRACCION TIPO CIUCANNI 506 {$xXx['CONTROL'] } ", $this->session);
                            case 396:
                            case 397:
...
//                                $check_fracciones_obligadas = $this->db->query("SELECT COUNT(*) AS EXISTE FROM fracpagnomina AS F WHERE F.numfrac IN(100,102,103,60,300) AND control = {$xXx['CONTROL']}")->result();
                                $check_fracciones_obligadas = $this->db->query("SELECT COUNT(*) AS EXISTE FROM fracpagnomina AS F WHERE F.numfrac IN(300,299) AND control = {$xXx['CONTROL']}")->result();
                                if (intval($check_fracciones_obligadas[0]->EXISTE) >= 1) {
                                    if (intval($xXx['DEPARTAMENTO']) === 140) {
                                        ...
                                        if (in_array(intval($REVISAR_AVANCE_POR_CONTROL[0]->EXISTE), array(0, 1)) && $FRACCION === 396 ||
                                                in_array(intval($REVISAR_AVANCE_POR_CONTROL[0]->EXISTE), array(0, 1)) && $FRACCION === 397 ||
                                                in_array(intval($REVISAR_AVANCE_POR_CONTROL[0]->EXISTE), array(0, 1)) && $FRACCION === 506) {
                                        ...
                                        switch (intval($check_tipo_de_construccion[0]->TIPO)) {
                                            case 4 && $FRACCION === 506:
                                                ...
                                                $l = new Logs("AVANCE 8", "HA AVANZADO EL CONTROL {$xXx['CONTROL']} A  - TEJIDO.   ", $this->session);
...
//                                                $l = new Logs("AVANCE 8", "HA AVANZADO EL CONTROL {$xXx['CONTROL']} A  - ALMACEN PESPUNTE.   ", $this->session);
                    ...
                    if ($FRACCION === 60 ||
                        ...
                        if (intval($control_muestra[0]->EXISTE) === 1 && $FRACCION === 60 ||
                                intval($control_muestra[0]->EXISTE) === 1 && $FRACCION === 61 ||
                                intval($control_muestra[0]->EXISTE) === 1 && $FRACCION === 71) {
                    ...
                    if ($check_fraccion[0]->EXISTE > 0 && intval($check_fraccion_plantilla[0]->EXISTE) <= 0 && $check_fraccion_fxe[0]->EXISTE > 0) {
                        if (intval($xXx['DEPARTAMENTO']) === 140) {
                            ...
                            if ($REVISAR_AVANCE_POR_CONTROL[0]->EXISTE === 0 && $FRACCION === 397) {
                                $avance = array(
                                    'Control' => $xXx['CONTROL'],
                                    'FechaAProduccion' => Date('d/m/Y'),
                                    'Departamento' => 130,
                            ...
                            if (intval($REVISAR_AVANCE[0]->EXISTE) >= 1 && $FRACCION === 397 && intval($xXx['DEPARTAMENTO']) === 140) {
    ...
    public function onRevisarCobroDeRayadoParaFOLEADO() {
        try {
            $x = $this->input->get();
            $avance_pago = array();
            $control = $this->db->query("SELECT P.Maquila FROM pedidox AS P WHERE P.Control = {$x['CONTROL']}")->result();
            ...
            switch (intval($control[0]->Maquila)) {
                case 1:
                    ...
                    break;
                case 98:
                    $sql = "SELECT P.Maquila, 2 AS EXISTE FROM pedidox AS P WHERE P.Control = {$x['CONTROL']}";
                    break;
            }
    ...
    public function onRevisarCobroDeEntreteladoParaALMCORTEOMAQUILA() {
        try {
            ...
            $sql_muestra = "SELECT COUNT(*) AS EXISTE FROM fracpagnomina AS F WHERE F.numfrac IN(96,113,114) AND F.control = {$x['CONTROL']} LIMIT 1";
            $control = $this->db->query("SELECT P.Maquila FROM pedidox AS P WHERE P.Control = {$x['CONTROL']}")->result();
            $revisa_cobro = $this->db->query($sql)->result();
            if (intval($revisa_cobro[0]->EXISTE) === 3) {
                ...
                if (intval($revisa_cobro[0]->EXISTE) === 3 && intval($revisa_cobro_entretelado[0]->EXISTE) === 0) {
                    ...
                    switch (intval($control[0]->Maquila)) {
                        case 98:
                            $sql_muestra = "SELECT COUNT(*) AS EXISTE FROM fracpagnomina AS F WHERE F.numfrac IN(96,113,114) AND F.control = {$x['CONTROL']} LIMIT 1";
                            $revisa_cobro = $this->db->query($sql_muestra)->result();
                            if (intval($revisa_cobro[0]->EXISTE) === 3) {
                                $avance_pago['COBRO_CORTE'] = "SI";
                                $avance_pago['COBRO_RAYADO'] = "SI";
                                $avance_pago['PUEDE_AVANZAR_A_ALMCORTEOMAQUILA'] = "SI";
    ...
    public function onAvance($CONTROL, $DEPARTAMENTO, $DEPARTAMENTOT, $FRACCION) {
        try {
            $check_avance = $this->db->query("SELECT COUNT(*) AS EXISTE FROM avance AS A WHERE A.Control = {$CONTROL} AND A.Departamento = {$DEPARTAMENTO}")->result();
