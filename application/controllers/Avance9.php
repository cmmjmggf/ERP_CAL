...
//            print json_encode($this->axepn->getSemanaByFecha(Date('d/m/Y')));
    ...
    public function onComprobarRetornoDeMaterialXControl() {
        try {
            $x = $this->input->get();
            if ($x['FR'] !== '' && intval($x['DEPTO']) === 10 || $x['FR'] !== '' && intval($x['DEPTO']) === 15 || $x['FR'] !== '' && intval($x['DEPTO']) === 370 || $x['FR'] !== '' && intval($x['DEPTO']) === 30) {
...
//                print $this->db->last_query();
                ...
                $control = $this->db->query("SELECT P.Maquila FROM pedidox AS P WHERE P.Control = {$x['CR']}")->result();
                if (intval($control[0]->Maquila) === 98) {
                ...
                switch (intval($x['DEPTO'])) {
                    ...
                    case 30:
                        ...
                        $this->db->where_in('F.Clave', array(103));
                        break;
                    case 50:
...
//            print json_encode($this->axepn->getUltimoAvanceXControl($x->get('C')));
            $control = $this->db->query("SELECT P.* FROM pedidox AS P WHERE P.Control = {$x['C']}")->result();
            switch (intval($control[0]->Maquila)) {
                case 98:
                    print json_encode($this->db->select("A.ID, A.Control, A.FechaProduccion AS FechaAProduccion, "
                                                    . "A.DeptoProduccion AS Departamento, A.EstatusProduccion AS DepartamentoT, DATE_FORMAT( A.FechaProduccion,\"%d/%m/%Y\") as FechaAvance, "
                                                    . "A.Estatus, 'MUESTRAS' AS Usuario,DATE_FORMAT( A.FechaProduccion,\"%d/%m/%Y\") AS Fecha  ", false)
                                            ->from('pedidox AS A')
                                            ->where("A.Control", $x['C'])
                                            ->limit(1)->get()->result());
                    break;
...
//            print json_encode($EMPLEADO_VALIDO);
            ...
            $DEPTOS_FISICOS = array(10/* CORTE */, 70, 20/* RAYADO */, 80/* RAYADO CONTADO */,
                30/* REBAJADO Y PERFORADO */, 180, 190/* MONTADO B */, 200, 210, 220, 300 /* SUPERVISORES */, 370);
            $xXx = $this->input->post();
                    ...
                    case 98:
                        switch (intval($xXx['NUMERO_EMPLEADO'])) {
                            case 2328:
                                ...
                                $avance_muestra['DepartamentoT'] = 'RAYADO';
                    ...
                    if (intval($check_avance[0]->EXISTE) <= 0) {
                        ...
                        if (intval($v->NUMERO_FRACCION) === 100 && intval($check_fraccion_plantilla[0]->EXISTE) <= 0 ||
                            ...
                            if (intval($REVISA_AVANCE_RAYADO[0]->EXISTE) === 0 && intval($check_fraccion_plantilla[0]->EXISTE) <= 0) {
                                ...
                                if (intval($check_programacion[0]->EXISTE) > 0 && $MAQUILA_MUESTRA !== 98) {
                                    $avance = array(
                                        'Control' => $xXx['CONTROL'],
                                        'FechaAProduccion' => Date('d/m/Y'),
                                        'Departamento' => 20,
                ...
                switch (intval($xXx['DEPARTAMENTO'])) {
                    ...
                    case 80:
                        ...
                        if (intval($revisar_avance[0]->EXISTE) === 2 && intval($check_fraccion_plantilla[0]->EXISTE) <= 0) {
                            $check_no_fremapeatamaat = $this->db->query("SELECT COUNT(*) AS EXISTE FROM fracpagnomina AS P WHERE P.Control = {$xXx['CONTROL']} AND numfrac IN(102,113,60,51,300,397,401,500,600)")->result();
                            if (intval($check_no_fremapeatamaat[0]->EXISTE) === 0) {
                                $l = new Logs("AVANCE 9", "  EL CONTROL  {$xXx['CONTROL']} HA SIDO AVANZADO DE RAYADO A FOLEADO. ", $this->session);
                                $this->onAvanzarXControl($xXx['CONTROL'], 'FOLEADO', 40, 4);
                            } else {
                                print "\n EL CONTROL YA HA SIDO COBRADO POR RAYADO \n";
                                $l = new Logs("AVANCE 9", "  EL CONTROL  {$xXx['CONTROL']} YA HA SIDO COBRADO POR RAYADO. ", $this->session);
                                exit(0);
                            }
                        }
                        /* FILTRADO POR FRACCION 102 RAYADO */
                        $FRACCION_CONTROL = $this->db->query("SELECT COUNT(*) AS EXISTE FROM  fracpagnomina where numfrac IN(113,102,80) and control = {$xXx['CONTROL']} LIMIT 1")->result();
                        ...
                        /* 80 CONTAR TAREA */
                        $FRACCION = 80;
                        $check_fraccionxestilo = $this->db->select('COUNT(*) AS EXISTE', false)
                                        ->from('fraccionesxestilo AS F')
                                        ->where('Estilo', $xXx['ESTILO'])
                                        ->where('Fraccion', $FRACCION)
                                        ->get()->result();
                        if (intval($check_fraccionxestilo[0]->EXISTE) > 0) {
                            $check_fraccion = $this->db->select('COUNT(F.numeroempleado) AS EXISTE', false)
                                            ->from('fracpagnomina AS F')
                                            ->where('F.control', $xXx['CONTROL'])
                                            ->where('F.numfrac', $FRACCION)
                    ...
                    case 30:
                        ...
                        $check_corte = $this->db->select('COUNT(*) AS EXISTE', false)
                                        ...
                                        ->where_in('F.numfrac', array(100, 96))
                                        ->get()->result();
                        ...
                        $check_rayado = $this->db->select('COUNT(*) AS EXISTE', false)
                                        ->from('fracpagnomina AS F')->where('F.control', $xXx['CONTROL'])
                                        ->where_in('F.numfrac', array(102, 113))
                                        ->get()->result();
...
//25101503
                                ...
                                if (intval($REVISAR_ENTRETELADO_X_ESTILO_CONTROL[0]->EXISTE) >= 1) {
                                    ...
                                    if (intval($check_avance[0]->EXISTE) === 0) {
                                        $check_fraccion = $this->db->query("SELECT COUNT(*) AS EXISTE FROM fracpagnomina AS F WHERE F.numfrac IN(127,51,300,397,401,500,600) AND F.control ={$xXx['CONTROL']}")->result();
                                        if (intval($check_fraccion[0]->EXISTE) === 0) {
                                            ...
                                            $this->db->insert('avance', array(
                                                ...
                                                'Hora' => Date('h:i:s a'), 'Fraccion' => $FRACCION, 'modulo' => 'A9'
                                            ));
                                            $id = $this->db->insert_id();
                                            $data["avance_id"] = intval($id) >= 0 ? intval($id) : 0;
                    ...
                    case 300:
                        ...
                        $check_avance = $this->db->query("SELECT COUNT(*) AS EXISTE FROM fracpagnomina AS F WHERE F.numfrac IN(300,397,401,500,600) AND F.control ={$xXx['CONTROL']}")->result();
                        if (intval($check_avance[0]->EXISTE) === 0) {
